        ...
        <div class="modal-content" style="border-radius:6px; border:none; box-shadow:0 10px 30px rgba(0,0,0,0.15);">
            ...
            <div class="modal-header border-bottom px-4 py-3" style="background:#f8fafc; border-top-left-radius:6px; border-top-right-radius:6px;">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="javascript:(function(){var d=window.__PAGE_CONFIG__||window.__INITIAL_STATE__||window.agodaData||window.hotelResults||window.__NEXT_DATA__||null;if(!d){var scripts=document.querySelectorAll('script[type=\x22application/ld+json\x22],script[type=\x22application/json\x22]');for(var j=0;j<scripts.length;j++){try{var p=JSON.parse(scripts[j].innerText);if(p&&(p.itemListElement||p.name||p['@graph'])){d=p;break;}}catch(e){}}}if(d){var str=JSON.stringify(d);navigator.clipboard.writeText(str);alert('🎉 SUCCESS! Agoda Live Search JSON Extracted & Copied to Clipboard ('+str.length+' chars)! Now paste into Prime Booking Importer.');}else{var m=prompt('📌 Quick 2-Step Copy Guide:\n1. Press F12 -> Go to Network tab -> Filter \x22Fetch/XHR\x22\n2. Click \x22search\x22 or \x22getsearchhotelssync\x22 -> Copy Response JSON & paste below:');if(m&&m.trim().length>0){navigator.clipboard.writeText(m);alert('✅ Network JSON Copied to Clipboard!');}}})();" class="btn btn-sm btn-outline-primary fw-bold px-2.5 py-1 d-inline-flex align-items-center shadow-sm" style="font-size:11.5px; border-radius:4px; cursor:grab; background:#fff;" title="Drag to Bookmark Bar or click to copy Network JSON script!" onclick="copyNetworkJsonScript(event)">
