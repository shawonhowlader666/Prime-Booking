    ...
    <div class="container" style="height: 50px; max-width: 1140px; margin: 0 auto; padding-left: 15px; padding-right: 15px; display: flex; align-items: center;">
        ...
        <div class="collapse navbar-collapse py-0 d-none d-lg-flex" id="agodaMainNav" style="height: 50px;">
            ...
            <div class="d-flex align-items-center ms-auto flex-shrink-0" style="gap: 16px; white-space: nowrap; height: 50px;">
                ...
                {{-- 4. Hamburger ☰ Menu Button with Agoda Screenshot 1 Exact Dropdown Card --}}
                <div class="dropdown position-relative">
                    ...
                    <div class="dropdown-menu dropdown-menu-end border-0 shadow-lg p-0 mt-2"
                        ...
                        <div style="padding: 16px 18px 8px;">
                            ...
                            @else
                            <div style="font-weight: 700; color: #1e293b; font-size: 13.5px; margin-bottom: 6px;">{{ auth()->user()->name }}</div>
                            <a href="{{ route('profile') }}" class="d-block text-decoration-none text-muted small mb-2">{{ __('Account Settings') }}</a>
                            <form action="{{ route('auth.logout') }}" method="POST" class="mb-3">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-light w-100 rounded-pill text-danger fw-bold py-1.5" style="border: 1px solid #e2e8f0;">{{ __('Sign Out') }}</button>
                            </form>
                            @endguest
                        </div>
